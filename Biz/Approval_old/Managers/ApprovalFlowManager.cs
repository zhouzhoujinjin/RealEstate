...
namespace Approval.Managers
{
  public class ApprovalFlowManager
  {
    ...
    private IApprovalFlowNode createApprovalFlowNode(int itemId, FlowNode node, IApprovalFlowNode previousNode)
    {
      ...
      {
        //审批工作流类型
        case Approval.Consts.FlowNodeTypeApproval:
        case "approver":
          //节点审批人大于1人，默认为或签，
          if (node.UserIds.Count > 1)
          {
            var logicNode = new LogicApprovalFlowNode
            {
              ...
              ActionType = Approval.ApprovalActionType.Created,
              //为节点的审批类型复制
              NodeType = ((ApprovalNode)node).CounterSign ? Approval.ApprovalFlowNodeType.And : Approval.ApprovalFlowNodeType.Or,
              ItemId = itemId,
        ...
        //工作流起始节点类型
        case Approval.Consts.FlowNodeTypeStart:
        ...
        //抄送节点类型
        case Approval.Consts.FlowNodeTypeCarbonCopy:
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<List<IApprovalFlowNode>> BuildFlowAsync(int templateId, Dictionary<string, string> formValues, int itemId)
    {
