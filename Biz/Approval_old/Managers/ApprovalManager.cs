...
namespace Approval.Managers
{
  public class ApprovalManager
  {
    ...
    private readonly SeafileManager seafileManager;
    ...
    public async Task<ApprovalItemEntity> UpdateItemInfo(ApprovalItemModel item)
    {
      ...
      foreach (var key in item.Content.Keys.ToList())
      {
        if (key.EndsWith("repoId"))
        {
          var parts = key.Split(".")[0..^1];
          var pathKey = string.Join(".", parts.Append("filePath"));
          var newPath = item.Content[pathKey].Replace("/0/", $"/{entity.Id}/");
          var urlKey = string.Join(".", parts.Append("url"));
          item.Content[urlKey] = item.Content[urlKey].Replace("/0/", $"/{entity.Id}/");
          if (newPath != item.Content[pathKey])
          {
            var i = newPath.LastIndexOf("/");
            await seafileManager.MoveFileAsync(item.TemplateName, item.Content[pathKey], newPath[0..i]);
            item.Content[pathKey] = newPath;
          }
        }
      }
    ...
    /// <returns></returns>
    public async Task<IEnumerable<BriefUser>> StartApproval(int itemId)
    {
    ...
    /// <param name="item"></param>
    /// <returns></returns>
    public async Task<List<BriefUser>> GetPendingUsers(ApprovalItemEntity item)
    {
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<List<BriefUser>> GetPendingUsers(int itemId)
    {
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<BriefUser> GetCreator(int itemId)
    {
      var item = await context.ApprovalItems.FirstOrDefaultAsync(x => x.Id == itemId);
      if (item == null) return null;
    ...
    /// <param name="itemId"></param>
    /// <returns></returns>
    public async Task<IEnumerable<BriefUser>> GetItemCcUsers(int itemId)
    {
      IEnumerable<BriefUser> ts = new List<BriefUser>();
    ...
    public async Task<List<BriefUser>> GetApprovalUsers(long creatorId, Dictionary<string, string> filters)
    {
      ...
      {
        var user = await GetBriefUserDepartment(userId);
        users.Add(user);
      }
      return users;
    }
    ...
    {
      // 还没到此节点的审批时机
      if (logicNode.ActionType == ApprovalActionType.Created)
      {
        return;
      }
    ...
    /// <returns></returns>
    public async Task<ApprovalItemEntity> CreateApprovalItem(string code, string title, int templateId, string templateName, long creatorId, Dictionary<string, string> content, ApprovalItemStatus status, bool isUpdate)
    {
      ...
      foreach (var key in content.Keys.ToList())
      {
        if (key.EndsWith("repoId"))
        {
          var parts = key.Split(".")[0..^1];
          var pathKey = string.Join(".", parts.Append("filePath"));
          var newPath = content[pathKey].Replace("/0/", $"/{entity.Id}/");
          var urlKey = string.Join(".", parts.Append("url"));
          content[urlKey] = content[urlKey].Replace("/0/", $"/{entity.Id}/");
          if (newPath != content[pathKey])
          {
            var i = newPath.LastIndexOf("/");
            await seafileManager.MoveFileAsync(templateName, content[pathKey], newPath[0..i]);
            content[pathKey] = newPath;
          }
        }
      }
    ...
    public async Task<ApprovalItemModel> GetItemInfo(int itemId, long userId = 0)
    {
      ...
      var creator = await GetBriefUserDepartment(item.CreatorId, false);
      ...
      foreach (var node in nodes)
      {
        if (node.NodeType == ApprovalFlowNodeType.And || node.NodeType == ApprovalFlowNodeType.Or)
        {
          ...
          {
            child.User = await GetBriefUserDepartment(child.UserId);
          }
          n.Children = children.Select(x => x as IApprovalFlowNode).ToList();
        }
        else
        {
          node.User = await GetBriefUserDepartment(node.UserId);
        }
      }
    ...
    public async Task<(int, IEnumerable<ApprovalItemModel>)> GetUserHistoryItems(long userId, Dictionary<string, string> filters, int page = 1, int size = 20)
    {
      ...
      {
        item.Creator = await GetBriefUserDepartment(item.CreatorId.Value, false);
    ...
    private IQueryable<ApprovalNodeEntity> GetNodeQuery(Dictionary<string, string> filters, long userId)
    {
      ...
      {
        var userIds = JsonConvert.DeserializeObject<List<long>>(filters["userIds"]);
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<BriefUser> GetBriefUserDepartment(long userId, bool isMajor = true)
    {
      return await cache.GetAsync($"{Consts.UserProfileWithDepartmentCacheKey}:{userId}_{(isMajor ? "OnlyMajor" : "All")}", async () =>
         {
           ...
           {
             var department = departments.FirstOrDefault().Department;
             if (isMajor)
             {
               var temp = departments.Where(x => x.IsUserMajorDepartment == isMajor).FirstOrDefault();
               department = temp != null ? temp.Department : null;
             }
             if (department != null)
             {
               user.Profiles["departmentName"] = department.Title;
               user.Profiles["departmentId"] = department.Id;
    ...
    private async Task<bool> IsFinal(int templateId)
    {
    ...
    /// <param name="userId"></param>
    /// <returns></returns>
    public async Task<ApprovalNodeEntity> InsertTransNode(int nodeId, long userId, string comment, BriefUser user)
    {
    ...
    public async Task<bool> IsUpdateCommentAsync(int nodeId, BriefUser user, string comment)
    {
    ...
    public async Task<(int, IEnumerable<ApprovalItemModel>)> GetVerifyItems(Dictionary<string, string> filters, int page = 1, int size = 20)
    {
      ...
      foreach (var item in items)
      {
        var briefUser = await userManager.GetBriefUserAsync(item.CreatorId.Value, new string[] { "public" });
        item.Creator = briefUser;
      }
    ...
    private List<AttachFile> GetReadOnlyList(List<AttachFile> files, string baseUrl)
    {
      ...
      {
        files.ForEach(o => o.Url = $"{baseUrl}{o.Url}");
        return files;
      }
      else
      {
    ...
    //public string FieldSwitch(string field, string fieldValue)
    //{
    //  var r;
    //  switch (field)
    //  {
        ...
        using (OfficeOpenXml.ExcelPackage package = new OfficeOpenXml.ExcelPackage(file))
        {
          ...
          foreach (var approvalItems in approvalItemsGroupBy)
          {
            ...
            foreach (var item in approvalItems)
            {
              ...
              var uaerName = user.Profiles["FullName"].ToString();
              worksheet.Cells[row, 1].Value = uaerName;
              var i = 2;
              foreach (var field in template.Fields)
              {
                if (field.ValueType != "fileList")
